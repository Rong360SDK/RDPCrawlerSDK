...
#import "RDPC_DatabasePool.h"
    ...
    #define RDPC_DBAutorelease(__v) ([__v autorelease]);
    #define RDPC_DBReturnAutoreleased RDPC_DBAutorelease

    #define RDPC_DBRetain(__v) ([__v retain]);
    #define RDPC_DBReturnRetained RDPC_DBRetain

    #define RDPC_DBRelease(__v) ([__v release]);

    #define RDPC_DBDispatchQueueRelease(__v) (dispatch_release(__v));
#else
    // -fobjc-arc
    #define RDPC_DBAutorelease(__v)
    #define RDPC_DBReturnAutoreleased(__v) (__v)

    #define RDPC_DBRetain(__v)
    #define RDPC_DBReturnRetained(__v) (__v)

    #define RDPC_DBRelease(__v)

// If OS_OBJECT_USE_OBJC=1, then the dispatch objects will be treated like ObjC objects
// and will participate in ARC.
// See the section on "Dispatch Queues and Automatic Reference Counting" in "Grand Central Dispatch (GCD) Reference" for details. 
    #if OS_OBJECT_USE_OBJC
        #define RDPC_DBDispatchQueueRelease(__v)
    #else
        #define RDPC_DBDispatchQueueRelease(__v) (dispatch_release(__v));
    #endif
#endif
...
typedef int(^RDPC_DBExecuteStatementsCallbackBlock)(NSDictionary *resultsDictionary);


/** A SQLite ([http://sqlite.org/](http://sqlite.org/)) Objective-C wrapper.
 
 ### Usage
 The three main classes in RDPC_DB are:

 - `FMDatabase` - Represents a single SQLite database.  Used for executing SQL statements.
 - `<RDPC_ResultSet>` - Represents the results of executing a query on an `FMDatabase`.
 ...
 - [RDPC_DB on GitHub](https://github.com/ccgus/fmdb) including introductory documentation
 - [SQLite web site](http://sqlite.org/)
 - [RDPC_DB mailing list](http://groups.google.com/group/fmdb)
...
@interface RDPC_Database : NSObject  {
...
/** Create a `FMDatabase` object.
 ...
 For example, to create/open a database in your Mac OS X `tmp` folder:
    ...
    RDPC_Database *db = [RDPC_Database databaseWithPath:@"/tmp/tmp.db"];

 Or, in iOS, you might open a database in the app's `Documents` directory:

    NSString *docsPath = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES)[0];
    NSString *dbPath   = [docsPath stringByAppendingPathComponent:@"test.db"];
    RDPC_Database *db     = [RDPC_Database databaseWithPath:dbPath];
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(RDPC_DBExecuteStatementsCallbackBlock)block;
...
/** Last insert rowid
 ...
 @see [sqlite3_last_insert_rdpcrowid()](http://sqlite.org/c3ref/last_insert_rdpcrowid.html)
...
/** Execute select statement
 ...
 Executing queries returns an `<RDPC_ResultSet>` object if successful, and `nil` upon failure.  Like executing updates, there is a variant that accepts an `NSError **` parameter.  Otherwise you should use the `<lastErrorMessage>` and `<lastErrorMessage>` methods to determine why a query failed.
 
 In order to iterate through the results of your query, you use a `while()` loop.  You also need to "step" (via `<[RDPC_ResultSet next]>`) from one record to the other.
 ...
 @return A `<RDPC_ResultSet>` for the result set upon success; `nil` upon failure. If failed, you can call `<lastError>`, `<lastErrorCode>`, or `<lastErrorMessage>` for diagnostic information regarding the failure.
 
 @see RDPC_ResultSet
 @see [`RDPC_ResultSet next`](<[RDPC_ResultSet next]>)
 @see [`sqlite3_bind`](http://sqlite.org/c3ref/bind_blob.html)
...
- (RDPC_ResultSet *)executeQuery:(NSString*)sql, ...;
...
- (RDPC_ResultSet *)executeQueryWithFormat:(NSString*)format, ... NS_FORMAT_FUNCTION(1,2);
...
- (RDPC_ResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments;
...
/** Execute select statement
 ...
 `func executeQuery(sql: String!, values: [AnyObject]!) throws  -> RDPC_ResultSet!`
...
- (RDPC_ResultSet *)executeQuery:(NSString *)sql values:(NSArray *)values error:(NSError * __autoreleasing *)error;
...
- (RDPC_ResultSet *)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary *)arguments;


// Documentation forthcoming.
- (RDPC_ResultSet *)executeQuery:(NSString*)sql withVAList: (va_list)args;
...
+ (NSString*)RDPC_DBUserVersion;

+ (SInt32)RDPC_DBVersion;
...
/** Adds SQL functions or aggregates or to redefine the behavior of existing SQL functions or aggregates.
 ...
 For example:
    ...
    [queue inDatabase:^(RDPC_Database *adb) {
        ...
        int rowCount = 0;
        RDPC_ResultSet *ars = [adb executeQuery:@"select * from ftest where StringStartsWithH(foo)"];
        while ([ars next]) {
            rowCount++;
            NSLog(@"Does %@ start with 'h'?", [rs stringForColumnIndex:0]);
        }
        RDPC_DBQuickCheck(rowCount == 2);
    }];
...
/** Generate an `NSDateFormatter` that won't be broken by permutations of timezones or locales.
 ...
 Example:
    ...
    myDB.dateFormat = [RDPC_Database storeableDateFormat:@"yyyy-MM-dd HH:mm:ss"];
 ...
 @warning Note that `NSDateFormatter` is not thread-safe, so the formatter generated by this method should be assigned to only one RDPC_DB instance and should not be used for other purposes.
...
/** Set to a date formatter to use string dates with sqlite instead of the default UNIX timestamps.
 ...
 @param format Set to nil to use UNIX timestamps. Defaults to nil. Should be set using a formatter generated using RDPC_Database::storeableDateFormat.
 ...
 @warning Note there is no direct getter for the `NSDateFormatter`, and you should not use the formatter you pass to RDPC_DB for other purposes, as `NSDateFormatter` is not thread-safe.
 */
...
/** Objective-C wrapper for `sqlite3_stmt`
 ...
 This is a wrapper for a SQLite `sqlite3_stmt`. Generally when using RDPC_DB you will not need to interact directly with `FMStatement`, but rather with `<FMDatabase>` and `<RDPC_ResultSet>` only.
...
@interface RDPC_Statement : NSObject {
    void *_statement;
    NSString *_query;
    long _useCount;
    BOOL _inUse;
