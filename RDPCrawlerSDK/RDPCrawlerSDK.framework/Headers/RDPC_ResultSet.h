...
@interface RDPC_ResultSet : NSObject {
    RDPC_Database          *_parentDB;
    RDPC_Statement         *_statement;
...
@property (atomic, retain) RDPC_Statement *statement;
...
/** Create result set from `<FMStatement>`
 ...
 @return A `RDPC_ResultSet` on success; `nil` on failure
 */

+ (instancetype)resultSetWithStatement:(RDPC_Statement *)statement usingParentDatabase:(RDPC_Database*)aDB;

/** Close result set */

- (void)close;

- (void)setParentDB:(RDPC_Database *)newDb;
